...
export function initCinematography({ state, capsule, audio, calloutUpdate, bloomPass }) {
  ...
  gsap.timeline({ scrollTrigger: { trigger: '#hero', start: 'top top', end: 'bottom bottom', scrub } })
    ...
    .to('#hero .copy', { opacity: 0, y: -30, duration: 0.18, ease: 'power2.in' }, 0.5)
    .to('.scroll-cue', { opacity: 0, duration: 0.1 }, 0.06);

  /* ========== 2 - INDUSTRIAL DESIGN: pirouette to the seamless monocoque ========== */
  gsap.timeline({ scrollTrigger: { trigger: '#design', start: 'top bottom', end: 'bottom bottom', scrub } })
  ...
  /* ========== 3 - HYGIENE: glide into the aperture ========== */
  gsap.timeline({
    scrollTrigger: {
      trigger: '#hygiene', start: 'top bottom', end: 'bottom bottom', scrub,
      onUpdate(st) {
        // callouts visible through the heart of the section
        const p = st.progress;
  ...
  const fireVIS = () => { if (!reduced) { audio.whoosh(); audio.shutter(); } setMode('vis'); };
  const firePOL = () => { if (!reduced) { audio.whoosh(); audio.shutter(); } setMode('pol'); };
  const fireUV = () => { if (!reduced) audio.uvShimmer(); setMode('uv'); };
  const clearModes = () => setMode(null);

  gsap.timeline({
    scrollTrigger: {
      trigger: '#optics', start: 'top bottom', end: 'bottom bottom', scrub,
      onLeaveBack: clearModes,
    },
  })
    // settle to a frontal, reverent view while ambient drops
  ...
  gsap.timeline({ scrollTrigger: { trigger: '#revenue', start: 'top bottom', end: 'bottom bottom', scrub } })
    ...
    .to(bloomPass, { strength: 0.42, duration: 0.15 }, 0.05)
    .call(clearModes, null, 0.06)
  ...
  gsap.timeline({ scrollTrigger: { trigger: '#partner', start: 'top bottom', end: 'top 20%', scrub } })
  ...
  if (!reduced) {
    ...
    const boundaries = () => {
      ...
      return [...document.querySelectorAll('.section')].map((s) => s.offsetTop / max).filter((b) => b <= 1);
    };
    let bounds = boundaries();
    ScrollTrigger.addEventListener('refresh', () => { bounds = boundaries(); });
    ScrollTrigger.create({
      start: 0, end: 'max',
      snap: {
        snapTo: (value) => {
          let best = value;
          for (const b of bounds) if (Math.abs(b - value) < 0.045 && Math.abs(b - value) < Math.abs(best - value)) best = b;
          return best;
        },
